...
func (coder *FileSrvCoder)Encode(obj interface{},buf *bytes.Buffer) error {
	var  encoder  IEncoder
	if coder.fuseBinary{
		var msgh codec.MsgpackHandle
		msgh.WriteExt = true
		encoder = codec.NewEncoder(buf,&msgh)
	}else{
...
type DxFileServer struct {
	dxserver.DxTcpServer
	fReadingFile	map[string]map[uint]*dxserver.DxNetConnection //正在读的文件列表
	fWritingFile	map[string]*dxserver.DxNetConnection //正在写的文件列表
	fLockingQueue	map[*dxserver.DxNetConnection]string //锁定了内容的锁定队列
	filedir		string
...
func (srv *DxFileServer)lockFile(filename string,ReadLock bool,con *dxserver.DxNetConnection,result *ResultPkg)  {
	...
	if ReadLock{ //读文件锁定，写入到文件正在读的列表中去,那么这个文件不可写锁定
		...
		if _,ok :=srv.fWritingFile[filename];ok{
			...
			conmap := make(map[uint]*dxserver.DxNetConnection)
...
func (srv *DxFileServer)unlockFile(filename string,ReadunLock bool,con *dxserver.DxNetConnection)  {
	srv.Lock()
...
func (srv *DxFileServer)download(filename string,fsize int64,position int64,result *ResultPkg,con *dxserver.DxNetConnection)  {
	...
	if position>=fsize{
		...
		file.Read(result.refData[:datalen])
		if coder.fuseBinary{
			result.Result = []uint8(result.refData[:datalen])
		}else{
			result.Result = base64.StdEncoding.EncodeToString(result.refData[:datalen]) //返回内容
		}
		file.Close()
	}else{
...
func (srv *DxFileServer)getUpdateVersion(versionFile string,result *ResultPkg,con *dxserver.DxNetConnection) {
	if file,err := os.Open(versionFile);err==nil{
		result.refData = srv.GetBuffer()
		_,err := file.Read(result.refData[:2]) //读取头部信息
		if err == nil && result.refData[0]=='U' && result.refData[1]=='P'{
			if _,err = file.Read(result.refData[2:4]);err == nil{//读取更新的文件数据类型的长度
				verdatalen := binary.BigEndian.Uint16(result.refData[2:4])
				if _,err = file.Read(result.refData[:verdatalen]);err == nil{//读取实际的版本数据
					result.Result = []uint8(result.refData[:verdatalen])
				}else{
					result.Err =  err.Error()
					result.Result = 3
				}
...
func (srv *DxFileServer)upload(filename string,position int64,con *dxserver.DxNetConnection,filedata interface{},result *ResultPkg)  {
	var  fileobj *os.File
	var err error
	if position == 0{
...
func NewFileServer(filedir string)*DxFileServer{
	...
	srv.fReadingFile = make(map[string]map[uint]*dxserver.DxNetConnection)
	srv.fWritingFile = make(map[string]*dxserver.DxNetConnection)
	srv.fLockingQueue =make(map[*dxserver.DxNetConnection]string)
	srv.SetCoder(coder)
	...
	srv.OnSendData = func(con *dxserver.DxNetConnection,Data interface{},sendlen int,sendok bool){
		//回收
	...
	srv.OnClientDisConnected = func(con *dxserver.DxNetConnection){
		//客户端断开，释放锁定的信息
		srv.Lock()
